...
from ortho.builders import get_orthonormal_basis_from_sample, OrthoBuilder
...
from mercergp.builders import MercerGPBuilder
...
if __name__ == "__main__":
    ...
    order = 12  # degree of approximation
    dim = 1
    sample_size = 4000
    ...
    mercer_gp_builder = (
        MercerGPBuilder(order)
        .set_parameters(kernel_args)
        .set_basis(mercer_basis)
    ...
    for i in range(experiment_count):
        for j in range(mixtures_count):
            ...
            mercer_gp = mercer_gp_builder.build()
            ...
            # )
            favard_gp = favard_gp_builder.set_basis(favard_basis).build()
            ...
            if mercer_kl > favard_kl:
                print(colored("Favard is better!", "green"))
            else:
                print(colored("Favard is worse!", "red"))
            kls[i, j, 0] = mercer_kl
            kls[i, j, 1] = favard_kl
