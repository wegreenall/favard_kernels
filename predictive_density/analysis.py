...
"""
This script conducts the analysis for the predictive density experiment, which generates the 
...
normal_input_kls = torch.load(
    "./experiment_data/kl_divergences_normal_test_inputs.pt"
)
mixture_input_kls = torch.load(
    "./experiment_data/kl_divergences_mixture_test_inputs.pt"
)
...
if log_data:
    ...
    normal_lqgraph_0 = torch.quantile(
        normal_input_kls[:, :end_order, 0], 0.25, dim=0
    )
    normal_hqgraph_0 = torch.quantile(
        normal_input_kls[:, :end_order, 0], 0.75, dim=0
    )
    normal_lqgraph_1 = torch.quantile(
        normal_input_kls[:, :end_order, 1], 0.25, dim=0
    )
    normal_hqgraph_1 = torch.quantile(
        normal_input_kls[:, :end_order, 1], 0.75, dim=0
    )
    mixture_meangraph_0 = torch.mean(
        mixture_input_kls[:, :end_order, 0], dim=0
    )
    mixture_meangraph_1 = torch.mean(
        mixture_input_kls[:, :end_order, 1], dim=0
    )
    mixture_lqgraph_0 = torch.quantile(
        mixture_input_kls[:, :end_order, 0], 0.25, dim=0
    )
    mixture_hqgraph_0 = torch.quantile(
        mixture_input_kls[:, :end_order, 0], 0.75, dim=0
    )
    mixture_lqgraph_1 = torch.quantile(
        mixture_input_kls[:, :end_order, 1], 0.25, dim=0
    )
    mixture_hqgraph_1 = torch.quantile(
        mixture_input_kls[:, :end_order, 1], 0.75, dim=0
    )
