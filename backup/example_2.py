...
from ortho.builders import (
    get_symmetric_orthonormal_basis,
...
def get_input_sample(sample_size):
    ...
    comp = D.Normal(torch.Tensor([-0, 4]), torch.Tensor([std, std]))
...
order = 8
end_point = 6
noise_parameter = true_noise_parameter.clone()
...
Set up the gammas
"""
gammas = get_gammas_from_sample(input_sample, order)
gammas[0] = 1
